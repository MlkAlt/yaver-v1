...
**Kütüphane:** Framer Motion (Hafta 1'de kurulur, sonradan eklemek 3x zor).
...
> "Design system: Bricolage Grotesque (display) + DM Sans (body). Colors: paper #FAF7F2, ink #1C1B17, sienna #B5481E. NEVER use Inter, Roboto, pure white, or purple gradients. Use design tokens from tailwind.config.js — no hard-coded values. One italic word per heading (micro-signature). Asymmetric layouts. Subtle noise overlay on backgrounds. Reference /refs folder for visual language. Animate key moments with Framer Motion (200-400ms, ease-out). Write emotional empty states. Vary icon sizes, spacing, text opacity. Every screen should answer: 'Would this look at home in Linear or Things 3?'"
...
19. UI iyileştirme + animasyonlar
20. PWA setup, deploy
...
5. [ ] Vite + React + TS + Tailwind projesini kur
6. [ ] Tailwind config'e design tokenları ekle (Bölüm G4)
7. [ ] Framer Motion kur
